...
def train_recons_a(original_imgs_path, validatioin_imgs_path, save_path_a, model_pre_path_a, ssim_weight_a, EPOCHES_set, BATCH_SIZE,MODEL_SAVE_PATHS, debug=False, logging_period=1):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        # ----------------------------------------------------------------------------------------------------------------
        loss_data = Loss_all[:count_loss]
        scio.savemat('/data/ljy/1-Project-Go/01-06-upsampling/models_a/loss/DeepDenseLossData' + str(ssim_weight_a) + '.mat',
                     {'loss': loss_data})

        loss_ssim_data = Loss_ssim[:count_loss]
        scio.savemat('/data/ljy/1-Project-Go/01-06-upsampling/models_a/loss/DeepDenseLossSSIMData' + str(
            ssim_weight_a) + '.mat', {'loss_ssim': loss_ssim_data})

        loss_pixel_data = Loss_pixel[:count_loss]
        scio.savemat('/data/ljy/1-Project-Go/01-06-upsampling/models_a/loss/DeepDenseLossPixelData.mat' + str(
            ssim_weight_a) + '', {'loss_pixel': loss_pixel_data})

        validation_ssim_data = Val_ssim_data[:count_loss]
        scio.savemat('/data/ljy/1-Project-Go/01-06-upsampling/models_a/val/Validation_ssim_Data.mat' + str(
            ssim_weight_a) + '', {'val_ssim': validation_ssim_data})

        validation_pixel_data = Val_pixel_data[:count_loss]
        scio.savemat('/data/ljy/1-Project-Go/01-06-upsampling/models_a/val/Validation_pixel_Data.mat' + str(
