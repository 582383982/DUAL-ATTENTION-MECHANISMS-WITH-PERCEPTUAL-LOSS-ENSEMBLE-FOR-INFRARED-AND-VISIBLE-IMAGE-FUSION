...
'''def _get_attention(enc_ir_block2_temp,enc_vis_block2_temp, model_path_a):
	g1 = tf.Graph()  # 加载到Session 1的graph
	sess1 = tf.Session(graph=g1)  # Session1
	...
	with sess1.as_default():
		with g1.as_default(), tf.Session() as sess:
			...
			print("-----------------------------------------------------------")
			print(type(enc_ir_block2_temp))
			print(enc_ir_block2_temp.shape)
			...
			return  enc_ir_block2_temp,enc_vis_block2_temp
...
def _handler_mix_a(ir_path, vis_path, model_path, model_pre_path,model_path_a,model_pre_path_a, ssim_weight, index, output_path=None):
	...
	with sess2.as_default():  # 1
		with g2.as_default(),tf.Session() as sess:
			...
			#enc_ir_block2_temp, enc_vis_block2_temp = _get_attention(enc_ir_block2_temp, enc_vis_block2_temp, model_path_a)
			...
			second = Strategy(enc_ir_res_block_temp[6], enc_vis_res_block_temp[6])
			...
			# output = sess.run(output_image, feed_dict={result: feature,enc_res_block:block,enc_res_block2:block2})
			#output = sess.run(output_image, feed_dict={result: feature})
			#output = output_image.eval()

			save_images(ir_path, output_image, output_path,
						prefix='fused' + str(index), suffix='_mix_' + str(ssim_weight))
...
def _get_attention(enc_ir_block2_temp,enc_vis_block2_temp, model_path_a):
	...
	with sess1.as_default():
		with g1.as_default(), tf.Session() as sess:
			...
			enc_ir_block2_temp,ir_att_temp = atn.get_attention(enc_ir_block2_temp)
			enc_vis_block2_temp,vis_att_temp = atn.get_attention(enc_vis_block2_temp)

			enc_ir_block2 = sess.run(enc_ir_block2_temp)
			enc_vis_block2 = sess.run(enc_vis_block2_temp)
			ir_att = sess.run(ir_att_temp)
			vis_att = sess.run(vis_att_temp)

			return  enc_ir_block2,enc_vis_block2,ir_att,vis_att
